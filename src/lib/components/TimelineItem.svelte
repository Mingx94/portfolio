<script lang="ts">
	...
	let liCls: string = clsx(liClasses[order], $$props.classLi);
	let divCls: string = clsx(divClasses[order], $$props.classDiv);
	let timeCls: string = clsx(timeClasses[order], $$props.classTime);
	const h3Cls = clsx(
		order === 'vertical'
